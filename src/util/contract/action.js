...
export const getGameOption=async (match,bet)=>{
    const {address}=match;
    if(address){
        const gameContract = getGameContract(address);
        const caller = await createMethodsCallOptions();
        const {optionId}=bet;
        const option=await gameContract.methods.options(optionId).call(caller);
...
export const placeBet=async (match,bet,amount)=>{
    // const address=await getMatchAddress(matchId);
    const {optionId,betId}=bet;
    const {address}=match;
    if(address)
    {
        try{
            ...
            const checkOptions=await getGameOption(match,bet);
            if(!checkOptions){
                return false;
            }
            ...
            const result=await gameContract.methods.payerBet([optionId], [amountStr]).send({
                ...sender,
                value: amountStr
            ...
            console.log(`payerBet-result`,result);
            const {transactionHash}=result;
            if(transactionHash){
